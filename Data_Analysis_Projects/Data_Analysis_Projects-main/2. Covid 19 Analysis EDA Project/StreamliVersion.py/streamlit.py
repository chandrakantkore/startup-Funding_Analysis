...
file1 = os.path.join(BASE_DIR,"Datasets",'covid19india.csv')
file2 = os.path.join(BASE_DIR, "Datasets",'covidvaccinestatewise.csv')
...
def load_data():
    ...
    # 1. Force convert columns to numeric, turning errors (text) into NaN
    vaccine_df["Male(Individuals Vaccinated)"] = pd.to_numeric(vaccine_df["Male(Individuals Vaccinated)"], errors='coerce')
    vaccine_df["Female(Individuals Vaccinated)"] = pd.to_numeric(vaccine_df["Female(Individuals Vaccinated)"], errors='coerce')
    vaccine_df["Male(Individuals Vaccinated)"].fillna(0, inplace=True)
    vaccine_df["Female(Individuals Vaccinated)"].fillna(0, inplace=True)
    male_vaccinated = vaccine_df["Male(Individuals Vaccinated)"].sum()
    female_vaccinated = vaccine_df["Female(Individuals Vaccinated)"].sum()
    male_col = "Male (Doses Administered)"
    female_col = "Female (Doses Administered)"

    # 2. Convert to numeric, coercing errors to NaN
    vaccine_df[male_col] = pd.to_numeric(vaccine_df[male_col], errors='coerce')
    vaccine_df[female_col] = pd.to_numeric(vaccine_df[female_col], errors='coerce')

    # 3. Logic Fix: The data is cumulative. 
    # We must take the MAX value for each state (the latest data) before summing.
    # Otherwise, we are summing the same people hundreds of times.
    vaccine_statewise = vaccine_df.groupby('State')[[male_col, female_col]].max()
    
    male_vaccinated = vaccine_statewise[male_col].sum()
    female_vaccinated = vaccine_statewise[female_col].sum()
    # --- FIX ENDS HERE ---
